...
import { listDeletedBlogCategories } from "@/modules/blog-taxonomy/api";
import { BlogCategoryTrashRowActionsMenu } from "@/modules/blog-taxonomy/ui/BlogCategoryTrashRowActionsMenu";

export const metadata = { title: "سطل زباله دسته‌بندی‌ها | پنل مدیریت" };
...
export default async function Page({
    ...
    const data = await listDeletedBlogCategories({ page, pageSize: 20, q });

    return (
        <AdminListPage<any>
            title="سطل زباله دسته‌بندی‌ها"
            basePath="/admin/blog-categories/trash"
            data={data}
            q={q}
            createButton={
                <Link
            ...
            rowMenuCell={(row) => (
                <BlogCategoryTrashRowActionsMenu id={row.id} title={row.name} />
            )}
            columns={[
