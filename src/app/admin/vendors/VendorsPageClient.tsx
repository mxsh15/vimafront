...
import type { VendorRow, PagedResult } from "@/modules/vendor/types";
import type { UserOptionDto } from "@/modules/user/api";
import { useVendors } from "@/modules/vendor/hooks";
import Link from "next/link";

export function VendorsPageClient(props: {
  data: PagedResult<VendorRow>;
  q: string;
  page: number;
  pageSize: number;
  status?: string;
  ...
  const list = vendorsQ.data ?? props.data;
  ...
  return (
    <AdminListPage<VendorRow>
      ...
      data={list}
      q={props.q}
      createButton={<VendorCreateButton userOptions={props.userOptions} />}
      ...
      totalLabel={`${list.totalCount ?? 0} فروشنده ثبت شده`}
      ...
      columns={[
        ...
        },
        {
          ...
          header: "مالک",
          cell: (r) => (
            <span className="text-sm text-slate-600">{r.ownerUserName || "-"}</span>
          ),
          cellClassName: "px-4",
        },
        {
